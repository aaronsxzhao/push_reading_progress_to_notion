...
PLIST_PATH="$HOME/Library/LaunchAgents/com.aaron.weread.notion.sync.plist"
PYTHON_BIN="$PROJECT_DIR/.venv/bin/python3"
SCRIPT_PATH="$PROJECT_DIR/src/weread_notion_sync.py"
...
: "${WEREAD_ROOT:?WEREAD_ROOT missing}"
...
<dict>
  <key>Label</key>
  <string>com.aaron.weread.notion.sync</string>
  ...
  <dict>
    ...
    <key>WEREAD_ROOT</key><string>${WEREAD_ROOT}</string>
  ...
  <string>/tmp/weread_notion_sync.out.log</string>
  <key>StandardErrorPath</key>
  <string>/tmp/weread_notion_sync.err.log</string>
</dict>
</plist>
EOF
...
launchctl start com.aaron.weread.notion.sync

echo "Installed & started: com.aaron.weread.notion.sync"
echo "Logs:"
echo "  tail -f /tmp/weread_notion_sync.out.log"
echo "  tail -f /tmp/weread_notion_sync.err.log"
